...
import { DEFAULT_END_DATE, DEFAULT_END_DATE_NAME, DEFAULT_NUMBER_DAYS } from '../../../../utils/contants';
...
export default class LogFile extends SfdxCommand {
  ...
  public async run(): Promise<AnyJson[]> {
    ...
    const fields: string[] = this.flags.fields;

    // Get metadata about event log files correspoding to the criteria
    const metadatas: EventLogMetadataEntry[] = await EventLogLogic.GetMetadata(this.org.getConnection(), eventType, startdate, enddate);
    ...
    const lines: AnyJson[] = await EventLogLogic.GetAllContent(this.org.getConnection(), metadatas, fields);
